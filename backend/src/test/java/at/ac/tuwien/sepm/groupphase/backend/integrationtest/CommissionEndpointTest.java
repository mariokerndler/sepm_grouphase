...
public class CommissionEndpointTest {
    ...
    public void addCommission_modifyAndDelete() throws Exception {
        ...
        Commission commission = Commission.builder().artist(artist1).customer(user1).status(CommissionStatus.LISTED).sketchesShown(3).feedbackSent(0).price(300).issueDate(LocalDateTime.now()).deadlineDate(LocalDateTime.now().plusDays(20)).title("its yours").instructions("do the thing").feedbackRounds(2).build();
        ...
        Commission modCommission = Commission.builder().id(commissionId).feedbackRounds(2).artist(artist1).customer(user1).status(CommissionStatus.LISTED).sketchesShown(3).feedbackSent(0).price(300).issueDate(commission.getIssueDate()).deadlineDate(commission.getDeadlineDate()).title("Drawing french").instructions("draw me like one of your french girls").build();
